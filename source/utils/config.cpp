...
void Config::initialize() {
	FILE *temp = fopen("sdmc:/3ds/Universal-Updater-Pablo/Config.json", "w");
	char tmp[2] = { '{', '}' };
...
*/
Config::Config() {
	if (access("sdmc:/3ds/Universal-Updater-Pablo/Config.json", F_OK) != 0) {
		this->initialize();
	}

	FILE *file = fopen("sdmc:/3ds/Universal-Updater-Pablo/Config.json", "rt");
	if (file) {
...
void Config::save() {
	if (this->changesMade) {
		FILE *file = fopen("sdmc:/3ds/Universal-Updater-Pablo/Config.json", "w");
