...
static void InitMusic() {
	if (access("sdmc:/3ds/dspfirm.cdc", F_OK) == 0) { // Ensure dspfirm dump exist.
		if (access("sdmc:/3ds/Universal-Updater-Pablo/music.wav", F_OK) == 0) { // Ensure music.wav exist.
			dspfirmFound = true;
			ndspInit();
			Music = std::make_unique<Sound>("sdmc:/3ds/Universal-Updater-Pablo/music.wav");
	...
	if (config->customfont()) {
		if (!needUnloadFont) {
			if (access("sdmc:/3ds/Universal-Updater-Pablo/font.bcfnt", F_OK) == 0) {
				Gui::loadFont(font, "sdmc:/3ds/Universal-Updater-Pablo/font.bcfnt");
				needUnloadFont = true;
			}
		}
	}
